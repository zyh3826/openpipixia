"""Provider resolution helpers for openheron runtime."""
...
def build_adk_model_from_env() -> Any:
    ...
    provider = normalize_provider_name(os.getenv("OPENHERON_PROVIDER"))
    model_name = normalize_model_name(provider, os.getenv("OPENHERON_MODEL"))
    ...
    if spec.runtime == RUNTIME_LITELLM:
        ...
        api_base = os.getenv("OPENHERON_PROVIDER_API_BASE", "").strip() or provider_default_api_base(provider)
        if api_base:
            kwargs["api_base"] = api_base

        raw_headers = os.getenv("OPENHERON_PROVIDER_EXTRA_HEADERS_JSON", "").strip()
        if raw_headers:
            try:
    ...
    if spec.runtime == RUNTIME_CODEX:
        ...
        codex_url = os.getenv("OPENHERON_PROVIDER_API_BASE", "").strip() or provider_default_api_base(provider)
